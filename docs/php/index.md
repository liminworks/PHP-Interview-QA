# PHP 语言篇

## L1 初级

| 频率 | 题目 |
|:---:|---|
| 高 | [PHP 8.0 的 match 表达式与 switch 有什么区别？](PHP8的match表达式与switch的区别.md) |
| 高 | [PHP 8.0 的 nullsafe 运算符（?->）是什么？与 ?? 有何区别？](PHP8的nullsafe运算符.md) |
| 高 | [PHP 中 == 和 === 的区别是什么](PHP中==和===的区别是什么.md) |
| 高 | [PHP 中引用传递与值传递的区别](PHP中引用传递与值传递的区别.md) |
| 高 | [PHP 索引数组与关联数组的区别及常见操作](PHP数组的索引数组与关联数组.md) |
| 高 | [PHP 浮点数精度问题是什么？如何用 bcmath 解决？](PHP浮点数精度问题与bcmath解决方案.md) |
| 高 | [declare(strict_types=1) 的作用是什么？严格模式与强制模式有何区别？](declare(strict_types=1)的作用与严格模式行为.md) |
| 高 | [isset() 与 empty() 有什么区别](isset与empty的区别.md) |
| 中 | [PHP 8.0 的命名参数（Named Arguments）是什么？有哪些使用场景？](PHP8的命名参数.md) |
| 中 | [PHP 动态语言特性与静态语言的区别](PHP动态语言特性与静态语言的区别.md) |
| 中 | [PHP 单引号与双引号字符串的区别](PHP单引号与双引号字符串的区别.md) |
| 中 | [PHP 类型强转有哪些方式？各种类型互转的规则是什么？](PHP类型强转规则与类型转换函数.md) |
| 中 | [PHP 的 never 返回类型表示什么？与 void 有何区别？](never返回类型与void的区别.md) |
| 中 | [PHP 的 union types（PHP 8.0）与交集类型（PHP 8.1）有何区别？各自有哪些使用约束？](union_types与交集类型的区别与约束.md) |

## L2 中级

| 频率 | 题目 |
|:---:|---|
| 高 | [Composer 自动加载机制与 PSR-4 规范](Composer自动加载机制与PSR-4规范.md) |
| 高 | [PHP 中 Error 与 Exception 的区别及异常处理最佳实践](Error与Exception的区别及异常处理最佳实践.md) |
| 高 | [PHP 命名空间的解析规则与别名机制](PHP命名空间的解析规则与别名机制.md) |
| 高 | [SOLID 设计原则是什么？各原则的核心意图是什么？](SOLID设计原则.md) |
| 高 | [PHP 中 Trait 的作用与冲突解决机制](Trait的作用与冲突解决机制.md) |
| 高 | [什么是依赖注入？它解决了什么问题？](依赖注入与控制反转.md) |
| 高 | [抽象类与接口的区别及使用场景](抽象类与接口的区别及使用场景.md) |
| 高 | [PHP 魔术方法是什么？__get/__set/__call 各适用于哪些场景？](魔术方法与属性方法重载.md) |
| 中 | [Late Static Binding 是什么及 static:: 与 self:: 的区别](LateStaticBinding与self和static的区别.md) |
| 中 | [PHP 多范式编程与场景选型](PHP多范式编程与场景选型.md) |

## L3 高级

| 频率 | 题目 |
|:---:|---|
| 高 | [OPcache 编译缓存的工作原理是什么？](OPcache编译缓存的工作原理.md) |
| 高 | [PHP 的垃圾回收机制是如何工作的](PHP垃圾回收机制.md) |
| 中 | [Fiber 与 Generator 有栈协程与无栈协程的本质区别](Fiber与Generator有栈协程与无栈协程的本质区别.md) |
| 中 | [Fiber 与 Swoole 协程的能力边界对比](Fiber与Swoole协程的能力边界对比.md) |
| 中 | [PHP JIT 是什么？Tracing 与 Function 两种模式有何区别？](PHP_JIT的工作模式与适用场景.md) |
| 中 | [PHP 请求生命周期的四个阶段](PHP请求生命周期的四个阶段.md) |
| 中 | [WeakReference 与 WeakMap 如何避免引用计数陷阱？](WeakReference与WeakMap避免引用计数陷阱.md) |
| 中 | [PHP zval 内部结构是什么？写时复制（COW）机制如何工作？](zval内部结构与写时复制机制.md) |
